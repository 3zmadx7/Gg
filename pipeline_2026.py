#!/usr/bin/env python3
"""
2026 EURUSD Pipeline: Dukascopy → Train → Backtest
====================================================
Downloads EURUSD M1 data for Jan-May 2026 via dukascopy-node,
resamples to all required timeframes, trains ML on Jan-Feb,
and backtests on Mar-May 2026.

Usage:
  python pipeline_2026.py                  # full run (download + train + backtest)
  python pipeline_2026.py --skip-download  # skip download, use existing data
  python pipeline_2026.py --csv path.csv   # use an existing Dukascopy M1 CSV
  python pipeline_2026.py --backtest-only  # only backtest using trained model
"""
...
def step1_download(csv_override: str | None = None) -> Path:
    """Download M1 EURUSD data via dukascopy-node; return path to CSV."""
    if csv_override:
        p = Path(csv_override)
        if not p.exists():
    ...
    existing = sorted(DUKA_RAW_DIR.glob("*.csv"))
    if existing:
    ...
    print("Downloading EURUSD M1 data (Jan-May 2026) from Dukascopy...")
    print(f"  {' '.join(cmd)}")
    ...
    csvs = sorted(DUKA_RAW_DIR.glob("*.csv"))
    if not csvs:
...
def step4_backtest(ensemble, feature_cols) -> dict:
    ...
    sl_floor     = config.risk.get("sl_pips", 30) * 0.0001   # minimum SL distance
    ...
    for i in range(WARMUP, len(rows)):
        ...
        if position is not None:
            ...
            if direction == TradeDirection.BUY.value:
                if lo <= sl:
                    pnl = (sl - position["entry"]) / 0.0001 * vol * 10
        ...
        if position is None and i < len(rows) - 5:
            ...
            if confidence >= 0.60 and signal in ("BUY", "SELL"):
                ...
                sl_pips_val  = dynamic_sl / 0.0001
                volume       = risk_amount / max(sl_pips_val * 0.0001 * 10, 1e-9)
    ...
    if position is not None:
        ...
        pnl = ((last["close"] - position["entry"]) if direction == TradeDirection.BUY.value
               else (position["entry"] - last["close"])) / 0.0001 * position["volume"] * 10
...
def step5_report(results: dict) -> None:
    ...
    if trades:
        saveable["trades_sample"] = [
            {k2: str(v2) if not isinstance(v2, (int, float, str)) else v2
             for k2, v2 in t.items()} for t in trades[:20]
        ]
...
def parse_args():
    p = argparse.ArgumentParser(description="2026 EURUSD Dukascopy → Train → Backtest pipeline")
...
def main():
    args = parse_args()
    banner("2026 EURUSD Pipeline  |  Train: Jan-Feb  |  Test: Mar-May")
    ...
    if not args.skip_download and not args.skip_convert and not args.backtest_only:
        banner("Step 1 — Download EURUSD M1 Data (Jan-May 2026)")
        csv_path = step1_download(csv_override=args.csv)
    elif args.csv:
        csv_path = Path(args.csv)
    else:
        existing = sorted(DUKA_RAW_DIR.glob("*.csv")) if DUKA_RAW_DIR.exists() else []
