#!/usr/bin/env python3
"""
2026 USDJPY Pipeline: Dukascopy → Train → Backtest
====================================================
Downloads USDJPY M1 data for Jan-May 2026 via dukascopy-node,
resamples to all required timeframes, trains ML on Jan-Feb,
and backtests on Mar-May 2026.

Usage:
  python pipeline_2026_USDJPY.py                  # full run (download + train + backtest)
  python pipeline_2026_USDJPY.py --skip-download  # skip download, use existing data
  python pipeline_2026_USDJPY.py --csv path.csv   # use an existing Dukascopy M1 CSV
  python pipeline_2026_USDJPY.py --backtest-only  # only backtest using trained model
"""
...
def step1_download(csv_override: str | None = None) -> Path:
    """Download M1 USDJPY data via dukascopy-node; return path to CSV."""
    if csv_override:
        p = Path(csv_override)
        if not p.exists():
    ...
    print("Downloading USDJPY M1 data (Jan-May 2026) from Dukascopy...")
    print(f"  {' '.join(cmd)}")
...
def parse_args():
    p = argparse.ArgumentParser(description="2026 USDJPY Dukascopy → Train → Backtest pipeline")
...
def main():
    args = parse_args()
    banner("2026 USDJPY Pipeline  |  Train: Jan-Feb  |  Test: Mar-May")
    ...
    if not args.skip_download and not args.skip_convert and not args.backtest_only:
        banner("Step 1 — Download USDJPY M1 Data (Jan-May 2026)")
